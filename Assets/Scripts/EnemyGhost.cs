...
public class EnemyGhost : MonoBehaviour
{
    ...
    private void Die()
    {
        if(Health <= 0)
        {
            ...
            isAlive = false;
            animatorController.Play("Ghost Die");
        }
        else
        {
            return;
    ...
    {
        Health -= damage; Debug.Log("Enemy took damage");
    ...
    private void DealDamage()
    {
        ...
        {
            enemiesToDamage[i].GetComponent<Player>().TakeDamage(dealsDamage); Debug.Log("Dealt Damage to Ghost: " + enemiesToDamage[i]);
        }
    }
